...
When Orca AI decides an action from your app should run, Orceum constructs and sends an authenticated HTTP request to your `base_url` + the action's `endpoint`. Your job is to handle it, execute the work, and return a clean response.
