...
<Note>
MCP apps use a different execution model compared to native apps. Authentication support is limited to `None` and `OAuth 2.0` — `API Key` is not supported for MCP.
</Note>
...
You can do this by clicking **Refresh Tools** in the **Orceum Developer Studio** under your app's Tools tab.
