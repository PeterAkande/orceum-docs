...
2. Based on tool descriptions, Orca AI selects the appropriate tool
3. Orceum calls `call_tool(tool_name, arguments)` on your server
4. The result is returned to Orca AI, which summarizes it for the user
...
After discovery, Orceum auto-generates a manifest from your tool descriptions. The quality of tool descriptions directly affects how well Orca AI selects and uses your tools. Write tool descriptions as if explaining to a non-technical user what the tool does.
...
<Tabs>
  ...
  <Tab title="OAUTH">
    Users go through an OAuth flow on first install. Orceum injects the access token into MCP requests as a Bearer token.
    ...
    Orceum automatically handles token refresh before calling your MCP server.
  </Tab>
</Tabs>
